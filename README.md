...
本工具集通过分析《雪中悍刀行》的文风基因，调用大语言模型生成完整的长篇武侠小说大纲、分章详细情节，并逐章自动撰写出符合目标字数与风格要求的正文。全程只需提供一本参考风格的小说原文（`.txt`），其余步骤全自动完成。
...
### 2️⃣ 下载/准备参考文本
- 准备《雪中悍刀行》的纯文本文件（`utf-8`编码），命名为 **`xuezhong.txt`**，放入项目根目录。
  > 你可以从合法渠道获取小说电子版，或使用任意你喜欢的武侠小说作为风格模板（需自行修改脚本中的文件名）。

### 3️⃣ 提取文风基因库
运行 `extract_style_genome.py` 脚本，它会将《雪中悍刀行》切块、逐块分析，最终生成 `style_genome.md`（内含词汇句式、描写手法、叙事技巧等规则）。
```bash
...
### ❌ 我想换一本参考小说（非《雪中悍刀行》）
→ 将你的小说文本命名为 `xuezhong.txt` 并替换，脚本会自动提取其风格。如果想保留多个风格文件，需修改 `extract_style_genome.py` 中的文件名。
...
**完成上述7个步骤后，你将得到一部160章、约100万字的完整武侠小说！** 祝你创作愉快 ✍️ 修改一下 加上配置环境的完整教程
